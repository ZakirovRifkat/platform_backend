...
def fillipov(vfields, jacobians, pfunction, tspan, y0, params, C, inopts):
    ...
    while not stopit:
        ...
        sol = solve_ivp(
            ...
            args=(vfields, jacobians, params, C, state, dir),
            method="DOP853",
            max_step=0.1,
            **options
        )
                ...
                if IE[k] == 4:
                    ...
                    if state[2] == 1:
                        ...
                        if IE[k] == 2 or IE[k] == 3:
                            ...
                            raise (
                                "Error, there is something wrong with the event in filippov"
                            )
                    elif state[3] == 1:
                        ...
                        raise (
                            "Error, There is something wrong with the state vector in filippov"
                        )
...
def filippovfunc(t, y, vfields, jacobians, params, C, state, dir):
    ...
    if state[0] == 1:
        ...
        raise (
            "Error, there is something wrong with the state vector in filippov:filippovfunc"
        )
    dy = F
    return dy
